...
class MultiDeviceManager {
  ...
  async scanForDevices() {
    try {
      ...
      console.log('🔍 Multi-device scan found:', guitarDevices.length, 'guitar devices');
      console.log('🔍 All guitar devices:', guitarDevices);

      // Check for new devices
      for (const portInfo of guitarDevices) {
        const deviceId = this.generateDeviceId(portInfo);
        if (!this.devices.has(deviceId)) {
          const device = {
            ...
            deviceName: null,
            getDisplayName: () => {
              // First priority: deviceName from boot.py (cleaned up)
              if (device.deviceName) {
                let name = device.deviceName;
                // Remove "BumbleGum Guitars - " prefix if present
                if (name.startsWith('BumbleGum Guitars - ')) {
                  name = name.substring('BumbleGum Guitars - '.length);
                }
                return this.makeNameUnique(name, device.id);
              }
              
              // Second priority: Extract from friendlyName
              if (device.portInfo.friendlyName) {
                let name = device.portInfo.friendlyName.replace(/\s*\(COM\d+\)$/, '');
                // Remove "BumbleGum Guitars - " prefix if present
                if (name.startsWith('BumbleGum Guitars - ')) {
                  name = name.substring('BumbleGum Guitars - '.length);
                }
                // If it's still generic, try to extract something useful
                if (name === 'USB Serial Device' || name.startsWith('USB Serial Device')) {
                  return this.makeNameUnique(`Guitar Device`, device.id);
                }
                return this.makeNameUnique(name, device.id);
              }
              
              // Fallback
              if (device.portInfo.manufacturer && device.portInfo.manufacturer.includes('CircuitPython')) {
                return this.makeNameUnique(`CircuitPython Device`, device.id);
              }
          ...
          console.log('📱 New device discovered:', device.getDisplayName());
          this.emit('deviceDiscovered', device);
          
          // Fetch device name in the background (non-blocking)
          setTimeout(() => {
            this.fetchDeviceName(device).then((name) => {
              if (name) {
                // Update UI after getting the name
                this.emit('deviceNameUpdated', device);
                
                // Check for auto-reconnection after getting device name
                this.checkAutoReconnection(device);
              }
            });
          }, 500); // Small delay to avoid interfering with other operations
          
          // Also check for auto-reconnection immediately for devices with known names
          setTimeout(() => {
            this.checkAutoReconnection(device);
          }, 1000);
        }
      ...
      // Check for removed devices (devices that have disappeared from system)
      const currentDeviceIds = guitarDevices.map(port => this.generateDeviceId(port));
      for (const [deviceId, device] of this.devices) {
        if (!currentDeviceIds.includes(deviceId)) {
          console.log(`🔄 Device ${device.getDisplayName()} disappeared from system (may be rebooting)`);
          
          if (device.isConnected) {
            // If this was the active device, preserve info for auto-reconnection
            if (this.activeDevice && this.activeDevice.id === deviceId) {
              console.log(`🔄 Active device ${device.getDisplayName()} disappeared, preserving info for auto-reconnection`);
              // Don't clear lastActiveDeviceInfo - let auto-reconnection handle it
            }
  ...
  generateDeviceId(portInfo) {
    return `${portInfo.vendorId}-${portInfo.productId}-${portInfo.serialNumber || portInfo.path}`;
  }

  makeNameUnique(baseName, excludeDeviceId) {
    // Count how many devices have the same base name (excluding the current device)
    const sameNameDevices = Array.from(this.devices.values())
    ...
    // If there are duplicates, append the product ID
    if (sameNameDevices.length > 0) {
      const device = Array.from(this.devices.values()).find(d => d.id === excludeDeviceId);
      if (device && device.portInfo.productId) {
        return `${baseName} (${device.portInfo.productId})`;
      }
    }
  ...
  async connectDevice(deviceId) {
    ...
    try {
      ...
      console.log(`🔌 Attempting to connect to ${device.getDisplayName()} on ${device.portInfo.path}`);
      ...
      return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(new Error(`Connection timeout for ${device.getDisplayName()}`));
        }, 5000); // 5 second timeout

        device.port.open((err) => {
          clearTimeout(timeout);
          if (err) {
            console.error(`❌ Failed to connect to ${device.getDisplayName()}:`, err);
            reject(err);
          } else {
            ...
            device.port.on('data', (data) => {
              const chunk = data.toString();
              this.emit('deviceData', chunk, device);
              
              // Handle legacy file processing if this device is active
              if (this.activeDevice && this.activeDevice.id === device.id && awaitingFile) {
                ...
                if (responseBuffers[awaitingFile].includes('END')) {
                  ...
                  if (!jsonMatch) {
                    ...
                    if (awaitingFile === 'config.json') {
                      ...
                      try {
                        device.port.write("READFILE:factory_config.json\n");
                  ...
                  try {
                    ...
                    if (awaitingFile === 'config.json') {
                      ...
                      try {
                        device.port.write("WRITEFILE:config.json\n");
                        device.port.write(JSON.stringify(parsed) + "\n");
                        device.port.write("END\n");
            ...
            device.port.on('error', (error) => {
              console.error(`Device ${deviceId} error:`, error);
              this.emit('deviceError', error, device);
            });

            device.port.on('close', () => {
            ...
            // If no active device, make this the active one
            if (!this.activeDevice) {
              this.setActiveDevice(device);
            }

            console.log(`✅ Connected to device ${device.getDisplayName()}`);
  ...
  async disconnectDevice(deviceId, isManualDisconnection = false) {
    const device = this.devices.get(deviceId);
    if (device && device.port && device.port.isOpen) {
      return new Promise((resolve) => {
        device.port.close(() => {
          ...
          // If this was the active device, handle it appropriately
          if (this.activeDevice && this.activeDevice.id === deviceId) {
            if (isManualDisconnection) {
              console.log(`🔄 Active device ${device.getDisplayName()} manually disconnected, clearing auto-reconnect info`);
              this.setActiveDevice(null, true); // Clear auto-reconnect info
            } else {
              console.log(`🔄 Active device ${device.getDisplayName()} disconnected, preserving info for auto-reconnection`);
              this.activeDevice = null;
              connectedPort = null;
              this.emit('activeDeviceChanged', null, device);
            }
          }
          
          console.log(`❌ Disconnected from device ${device.getDisplayName()}`);
          this.emit('deviceDisconnected', device);
  ...
  setActiveDevice(device, clearAutoReconnectInfo = false) {
    ...
    if (device && device.isConnected) {
      this.lastActiveDeviceInfo = {
        ...
        portPath: device.portInfo.path,
        serialNumber: device.portInfo.serialNumber,
        vendorId: device.portInfo.vendorId,
        productId: device.portInfo.productId,
        friendlyName: device.portInfo.friendlyName
      };
      console.log('💾 Stored active device info for auto-reconnection:', this.lastActiveDeviceInfo);
    } else if (!device && clearAutoReconnectInfo) {
      // Only clear stored info when explicitly requested (manual disconnection)
      this.lastActiveDeviceInfo = null;
      console.log('🗑️ Cleared active device info (manual disconnection)');
    } else if (!device) {
  ...
  getConnectedDevices() {
    // Only return devices that are actually connected (have open ports)
    const connectedDevices = Array.from(this.connectedDevices.values()).filter(device => 
      device.isConnected && device.port && device.port.isOpen
    );
    
    // Clean up stale entries from connectedDevices map
    for (const [deviceId, device] of this.connectedDevices) {
      if (!device.isConnected || !device.port || !device.port.isOpen) {
        this.connectedDevices.delete(deviceId);
        console.log(`🧹 Cleaned up stale connection entry for ${device.getDisplayName()}`);
      }
    }
  ...
  async fetchDeviceName(device) {
    // If we already have the name, don't fetch again
    if (device.deviceName) return device.deviceName;
    
    // Don't interfere with active connections
    if (device.isConnected || this.activeDevice === device) return null;
    
    // Check if this device is currently being used by the legacy system
    if (connectedPort && connectedPort.path === device.portInfo.path) return null;
    
    try {
      console.log(`📝 Fetching device name for ${device.portInfo.path}...`);
      ...
      return new Promise((resolve) => {
        const timeout = setTimeout(() => {
          ...
          console.log(`⏱️ Timeout fetching name for ${device.portInfo.path}`);
          resolve(null);
        ...
        tempPort.open((err) => {
          if (err) {
            clearTimeout(timeout);
            console.log(`❌ Failed to temporarily connect for name fetch: ${err.message}`);
            resolve(null);
            return;
          }

          let buffer = '';
          tempPort.write("READFILE:boot.py\n");
            ...
            if (buffer.includes('END')) {
              ...
              tempPort.close(() => {
                if (interfaceNameMatch) {
                  const name = interfaceNameMatch[1];
                  device.deviceName = name;
                  console.log(`📝 Got device name: ${name} for ${device.portInfo.path}`);
                  resolve(name);
                } else {
                  console.log(`❌ No device name found in boot.py for ${device.portInfo.path}`);
                  resolve(null);
                }
              });
            }
          ...
          tempPort.on('error', (error) => {
            clearTimeout(timeout);
            console.log(`❌ Error during name fetch: ${error.message}`);
            if (tempPort.isOpen) {
              tempPort.close();
            }
            resolve(null);
      ...
      console.log(`❌ Exception during device name fetch: ${error.message}`);
      return null;
    }
  }

  // Refresh device name for connected devices (bypasses connection checks)
  refreshConnectedDeviceName(device) {
    if (!device.isConnected) {
      console.log('❌ Cannot refresh name for disconnected device');
      return;
    }

    console.log('📝 Refreshing device name for connected device:', device.portInfo.path);
    
    // Use the active connection to read the device name
    if (device.port && device.port.isOpen) {
      let buffer = '';
      const onData = (data) => {
        buffer += data.toString();
        if (buffer.includes('END')) {
          device.port.off('data', onData);
          ...
          if (interfaceNameMatch) {
            ...
            device.deviceName = name;
            console.log(`📝 Refreshed device name: ${oldName} → ${name}`);
            this.emit('deviceNameUpdated', device);
          } else {
            console.log(`❌ No device name found in boot.py during refresh`);
          }
        }
      };
      
      device.port.on('data', onData);
      device.port.write("READFILE:boot.py\n");
  ...
  startAutoScan(intervalMs = 3000) {
    if (this.scanning) this.stopAutoScan();
    
    this.scanning = true;
    console.log('🔄 Starting multi-device auto-scan every', intervalMs, 'ms');
  ...
  isConnected() {
    return this.activeDevice && this.activeDevice.isConnected;
  }

  getPort() {
    return this.activeDevice ? this.activeDevice.port : null;
  }

  // Auto-reconnection logic
  async checkAutoReconnection(device) {
    // Only attempt auto-reconnection if we have stored device info
    if (!this.lastActiveDeviceInfo) {
      return;
    }

    // If we have an active device that's still connected, don't auto-reconnect
    if (this.activeDevice && this.activeDevice.isConnected) {
      return;
    }

    // Check if this device matches our stored active device info
    const matches = this.deviceMatches(device, this.lastActiveDeviceInfo);
    if (matches) {
      console.log('🔄 Auto-reconnecting to previously active device:', device.getDisplayName());
      console.log('🔄 Stored device info was:', this.lastActiveDeviceInfo);
      try {
        ...
        if (device.isConnected) {
          ...
          // Clear cached device name to force refresh
          device.deviceName = null;
          console.log('📝 Cleared cached device name to fetch fresh name after auto-reconnection');
          
          // Get the expected device name from stored info for the toast
          const expectedDeviceName = this.lastActiveDeviceInfo.deviceName;
          let displayName = expectedDeviceName;
          if (displayName && displayName.startsWith('BumbleGum Guitars - ')) {
            displayName = displayName.substring('BumbleGum Guitars - '.length);
          }
          
          // Force refresh the device name by reading from the connected device
          setTimeout(() => {
            this.refreshConnectedDeviceName(device);
          }, 1000); // Small delay to ensure device is fully ready
          
          // Show toast with expected device name immediately
          showToast(`Auto-reconnected to ${displayName || 'Device'}`, 'success');
          
          // Update stored info with current device info instead of clearing it
          // This allows subsequent renames/reconnections to work
          this.lastActiveDeviceInfo = {
            deviceName: device.deviceName,
            portPath: device.portInfo.path,
            serialNumber: device.portInfo.serialNumber,
          ...
          console.log('💾 Updated stored device info after auto-reconnection:', this.lastActiveDeviceInfo);
        }
      } catch (error) {
        console.error('❌ Auto-reconnection failed:', error);
      }
    } else {
      console.log('🔄 Device does not match stored info for auto-reconnection');
      console.log('🔄 Device info:', {
        name: device.deviceName,
        path: device.portInfo.path,
        serial: device.portInfo.serialNumber
      });
      console.log('🔄 Stored info:', this.lastActiveDeviceInfo);
    }
  }

  // Check if a device matches the stored device info for auto-reconnection
  deviceMatches(device, storedInfo) {
    console.log('🔄 Checking device match for auto-reconnection:');
    console.log('🔄 Current device:', {
      name: device.deviceName,
      path: device.portInfo.path,
      serial: device.portInfo.serialNumber,
      vendor: device.portInfo.vendorId,
      product: device.portInfo.productId,
      friendly: device.portInfo.friendlyName
    });
    console.log('🔄 Stored info:', storedInfo);
    
    // First try exact port path match (most reliable, but may change on reboot)
    if (device.portInfo.path === storedInfo.portPath) {
      console.log('✅ Match found: Port path');
      return true;
    }

    // Then try serial number match (if available)
    if (storedInfo.serialNumber && device.portInfo.serialNumber === storedInfo.serialNumber) {
      console.log('✅ Match found: Serial number');
      return true;
    }

    // Try device name match (if both devices have names)
    if (storedInfo.deviceName && device.deviceName && 
        storedInfo.deviceName === device.deviceName) {
      console.log('✅ Match found: Device name');
      return true;
    }

    // Try friendly name match (Windows device name)
    if (storedInfo.friendlyName && device.portInfo.friendlyName && 
        storedInfo.friendlyName === device.portInfo.friendlyName) {
      console.log('✅ Match found: Friendly name');
      return true;
    }

    // Try vendor/product ID combination (for devices without serial numbers)
    if (storedInfo.vendorId && storedInfo.productId &&
        device.portInfo.vendorId === storedInfo.vendorId &&
        device.portInfo.productId === storedInfo.productId) {
      console.log('✅ Match found: Vendor/Product ID (less reliable)');
      return true;
    }
...
// ===== MULTI-DEVICE UI FUNCTIONS =====
function initializeDeviceSelector() {
  // Track last device action timestamp
  window._lastDeviceAction = 0;
  // Track dropdown open state
  window._deviceDropdownWasOpen = false;
  const deviceSelectorButton = document.getElementById('deviceSelectorButton');
  const deviceDropdown = document.getElementById('deviceDropdown');
  const deviceList = document.getElementById('deviceList');
  const refreshDevicesBtn = document.getElementById('refreshDevicesBtn');

  if (!deviceSelectorButton || !deviceDropdown || !deviceList || !refreshDevicesBtn) {
    console.warn('Device selector UI elements not found, multi-device UI disabled');
    return;
  }

  // Toggle dropdown
  deviceSelectorButton.addEventListener('click', () => {
    deviceDropdown.style.display = deviceDropdown.style.display === 'block' ? 'none' : 'block';
  });

  // Prevent dropdown from closing on device action DOM update
  window.ignoreNextDropdownClose = false;
  document.addEventListener('click', (event) => {
    // Prevent closing if a device action just occurred (within 300ms)
    const now = Date.now();
    if (window._lastDeviceAction && (now - window._lastDeviceAction < 300)) {
      return;
    }
    let target = event.target;
    while (target) {
      if (target.classList && target.classList.contains('device-btn')) {
        return;
      }
      target = target.parentElement;
    }
    if (!deviceSelectorButton.contains(event.target) && !deviceDropdown.contains(event.target)) {
  ...
  refreshDevicesBtn.addEventListener('click', () => {
    ...
    multiDeviceManager.scanForDevices();
  });

  // Multi-device manager event listeners
  multiDeviceManager.on('deviceDiscovered', (device) => {
    updateDeviceList();
  });

  multiDeviceManager.on('deviceRemoved', (device) => {
    updateDeviceList();
  });

  multiDeviceManager.on('deviceNameUpdated', (device) => {
    updateDeviceList();
    updateDeviceSelector();
  });

  multiDeviceManager.on('deviceConnected', (device) => {
    updateDeviceList();
    updateDeviceSelector();
    updateConnectionStatus();
  });

  multiDeviceManager.on('deviceDisconnected', (device) => {
    updateDeviceList();
    updateDeviceSelector();
    updateConnectionStatus();
  });
  ...
  multiDeviceManager.on('activeDeviceChanged', (newDevice, previousDevice) => {
    ...
    updateDeviceSelector();
    updateConnectionStatus();
    console.log('🎯 Active device changed to:', newDevice ? newDevice.getDisplayName() : 'None');
    
    // Update legacy connectedPort for backward compatibility
    if (newDevice && newDevice.isConnected) {
      connectedPort = newDevice.port;
      console.log('🔄 Updated legacy connectedPort to:', newDevice.getDisplayName());
      
      // Set up legacy data processing for this device
      if (connectedPort && connectedPort.writable) {
        console.log('🔄 Active device changed, requesting device info...');
        
        // Reset file reading state for new device
        awaitingFile = null;
        responseBuffers = {};
        
        // Initialize file reading for the new active device with a small delay
      ...
      connectedPort = null;
      console.log('🔄 Cleared legacy connectedPort');
      updateStatus('Disconnected');
      
      // Clear file reading state
      awaitingFile = null;
      responseBuffers = {};
      readQueue = [];
    }
  ...
  // Check for existing legacy connection first
  if (connectedPort && connectedPort.isOpen) {
    console.log('🔍 Found existing legacy connection, attempting to integrate...');
    // Try to find this device in our scan and mark it as connected
    setTimeout(() => {
      multiDeviceManager.scanForDevices().then(() => {
        // Look for a device that matches the current connected port
        for (const [deviceId, device] of multiDeviceManager.devices) {
          if (device.portInfo.path === connectedPort.path) {
            console.log('✅ Found matching device for legacy connection:', device.getDisplayName());
    ...
    // No existing connection, do initial scan
    multiDeviceManager.scanForDevices();
  }
  
  // Also periodically check for legacy connection synchronization
  setInterval(() => {
    // If there's a legacy connection but no active device in multi-device system
    if (connectedPort && connectedPort.isOpen && !multiDeviceManager.activeDevice) {
      console.log('🔄 Found unsynchronized legacy connection, attempting integration...');
      multiDeviceManager.scanForDevices().then(() => {
        for (const [deviceId, device] of multiDeviceManager.devices) {
          if (device.portInfo.path === connectedPort.path) {
            console.log('✅ Late integration of legacy connection:', device.getDisplayName());
            device.port = connectedPort;
            device.isConnected = true;
            multiDeviceManager.connectedDevices.set(deviceId, device);
            multiDeviceManager.setActiveDevice(device);
    ...
    // If multi-device has an active device but no legacy connection
    else if (multiDeviceManager.activeDevice && (!connectedPort || !connectedPort.isOpen)) {
      console.log('🔄 Multi-device active but no legacy connection, clearing active device');
  ...
  multiDeviceManager.startAutoScan(3000);
}

// Global functions for device actions (called from HTML)
window.connectDevice = async (deviceId) => {
  try {
    console.log(`🔌 User requested connection to device: ${deviceId}`);
    const device = multiDeviceManager.devices.get(deviceId);
    if (device) {
      showToast(`Connecting to ${device.getDisplayName()}...`, 'info');
    }
    await multiDeviceManager.connectDevice(deviceId);
    if (device) {
      showToast(`Connected to ${device.getDisplayName()}`, 'success');
    }
  } catch (error) {
    console.error('Failed to connect device:', error);
    let errorMessage = error.message;
    if (errorMessage.includes('Access denied')) {
      errorMessage = 'Access denied - device may be in use by another application';
    } else if (errorMessage.includes('timeout')) {
      errorMessage = 'Connection timeout - device may not be responding';
    }
    showToast(`Failed to connect: ${errorMessage}`, 'error');
  }
};

window.disconnectDevice = async (deviceId) => {
  try {
    console.log(`🔌 User requested disconnection from device: ${deviceId}`);
    const device = multiDeviceManager.devices.get(deviceId);
    if (device) {
      showToast(`Disconnecting from ${device.getDisplayName()}...`, 'info');
    }
    await multiDeviceManager.disconnectDevice(deviceId, true); // Manual disconnection
    if (device) {
      showToast(`Disconnected from ${device.getDisplayName()}`, 'success');
    }
  } catch (error) {
...
// Global identify device function
window.identifyDevice = async (deviceId) => {
  try {
    console.log('🔍 Identifying device:', deviceId);
    const device = multiDeviceManager.devices.get(deviceId);
    if (!device) {
    ...
    if (!device.isConnected) {
      console.warn('Device not connected:', device.getDisplayName());
      showToast(`${device.getDisplayName()} is not connected`, 'error');
      return;
    }
    
    if (!device.port) {
      console.warn('Device port unavailable:', device.getDisplayName());
      showToast(`${device.getDisplayName()} port unavailable`, 'error');
      return;
    }

    if (!device.port.isOpen) {
      console.warn('Device port not open:', device.getDisplayName());
      showToast(`${device.getDisplayName()} port not open`, 'error');
      return;
    }

    console.log('✅ Device identification starting for:', device.getDisplayName());
    showToast(`Identifying ${device.getDisplayName()}...`, 'info');

    // Flash green fret 5 times
    for (let i = 0; i < 5; i++) {
      console.log(`🔍 Flash ${i + 1}/5 - turning green LED on`);
      // Turn on green LED (LED index 0 = green fret)
      device.port.write('SETLED:0:0:255:0\n');
      await new Promise(resolve => setTimeout(resolve, 300));
      
      console.log(`🔍 Flash ${i + 1}/5 - turning green LED off`);
      // Turn off green LED
      device.port.write('SETLED:0:0:0:0\n');
      await new Promise(resolve => setTimeout(resolve, 300));
    }
    
    console.log('✅ Device identification completed');
    showToast(`${device.getDisplayName()} identified`, 'success');
  } catch (error) {
    console.error('❌ Error identifying device:', error);
    showToast(`Failed to identify device: ${error.message}`, 'error');
  }
};

window.setActiveDevice = (deviceId) => {
  console.log(`🎯 Attempting to set active device: ${deviceId}`);
  const device = multiDeviceManager.devices.get(deviceId);
  
  if (device && device.isConnected) {
    console.log(`🎯 Setting device as active: ${device.getDisplayName()}`);
    multiDeviceManager.setActiveDevice(device);
    updateDeviceList();
    updateDeviceSelector();
    showToast(`Activated ${device.getDisplayName()}`, 'success');
  } else {
    console.log(`🎯 Cannot set active device - device not found or not connected`);
    if (!device) {
      showToast(`Device not found: ${deviceId}`, 'error');
    } else if (!device.isConnected) {
      showToast(`Device not connected: ${device.getDisplayName()}`, 'error');
    }
  }
};

function updateDeviceList() {
  // Restore dropdown open state if needed
  const deviceDropdown = document.getElementById('deviceDropdown');
  if (window._deviceDropdownWasOpen && deviceDropdown) {
  ...
  if (devices.length === 0) {
    ...
    item.className = 'device-item disabled';
  ...
  devices.sort((a, b) => a.getDisplayName().localeCompare(b.getDisplayName()));
  ...
  devices.forEach(device => {
    ...
    if (multiDeviceManager.activeDevice && multiDeviceManager.activeDevice.id === device.id) {
      item.classList.add('active');
    }

    const statusIcon = device.isConnected ? '🟢' : '🔴';
    
    item.innerHTML = `
      <span class="device-name">${statusIcon} ${device.getDisplayName()}</span>
      <div class="device-actions">
        ${device.isConnected ? 
          `<button onclick="window._lastDeviceAction=Date.now();disconnectDevice('${device.id}')" class="device-btn disconnect">Disconnect</button>` :
          `<button onclick="window._lastDeviceAction=Date.now();connectDevice('${device.id}')" class="device-btn connect">Connect</button>`
        }
        ${device.isConnected ? 
          `<button onclick="window._lastDeviceAction=Date.now();setActiveDevice('${device.id}')" class="device-btn ${multiDeviceManager.activeDevice && multiDeviceManager.activeDevice.id === device.id ? 'active' : ''}">
            ${multiDeviceManager.activeDevice && multiDeviceManager.activeDevice.id === device.id ? 'Active' : 'Select'}
          </button>
          <button onclick="window._lastDeviceAction=Date.now();identifyDevice('${device.id}')" class="device-btn identify">Identify</button>` : ''
        }
      </div>
    `;
...
function updateDeviceSelector() {
  const deviceSelectorButton = document.getElementById('deviceSelectorButton');
  if (!deviceSelectorButton) return;

  const activeDevice = multiDeviceManager.getActiveDevice();
  const connectedCount = multiDeviceManager.getConnectedDevices().length;
  
  if (activeDevice) {
    deviceSelectorButton.innerHTML = `${activeDevice.getDisplayName()} (${connectedCount} connected)`;
  } else if (connectedCount > 0) {
    deviceSelectorButton.innerHTML = `${connectedCount} connected (none active)`;
  } else {
    deviceSelectorButton.innerHTML = 'Click to connect a device';
  }
}
...
function updateConnectionStatus() {
  ...
  if (connectButton) {
    if (activeDevice && activeDevice.isConnected) {
      ...
      connectButton.textContent = 'Connect Guitar';
      connectButton.disabled = false;
...
// ===== Core file reading and device communication functions =====
const requestNextFile = () => {
  awaitingFile = readQueue.shift();
  console.log('📂 Requesting next file:', awaitingFile);
  console.log('📂 Connected port available:', !!connectedPort);
  console.log('📂 Remaining queue:', readQueue);
  
  if (awaitingFile && connectedPort) {
    responseBuffers[awaitingFile] = '';
    console.log('📂 Writing READFILE command for:', awaitingFile);
    ...
    requestDeviceName(name => {
      ...
      if (footerDeviceName) {
        if (name) {
          // Clean up the device name - remove "BumbleGum Guitars - " prefix
          let displayName = name;
          if (displayName.startsWith('BumbleGum Guitars - ')) {
            displayName = displayName.substring('BumbleGum Guitars - '.length);
          }
          footerDeviceName.textContent = displayName;
          footerDeviceName.style.color = '#2ecc40'; // Green color for connected
        } else {
          footerDeviceName.textContent = 'Unknown device';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  multiDeviceManager.on('activeDeviceChanged', (newDevice, previousDevice) => {
    if (newDevice) {
      ...
      updateStatus(`Connected: ${newDevice.getDisplayName()}`, true);
        ...
        if (connectedPort) {
          ...
          requestDeviceName(name => {
            if (name && newDevice) {
              ...
              if (footerDeviceName) {
                footerDeviceName.textContent = displayName;
                footerDeviceName.style.color = '#2ecc40'; // Green for connected
              }
              
              // Update header status with device name
              updateStatus(`Connected: ${displayName}`, true);
            }
          });
