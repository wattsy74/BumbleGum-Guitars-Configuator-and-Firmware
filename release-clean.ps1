...
function Create-GitHubRelease {
    ...
    $exePath = "out\BGG Configurator-win32-x64\BGG Configurator.exe"
    if (-not (Test-Path $exePath)) {
        ...
        $possiblePaths = @(
            "out\*\BGG Configurator.exe",
            "out\make\squirrel.windows\*\BGG Configurator.exe",
            "dist\*\BGG Configurator.exe"
        )
